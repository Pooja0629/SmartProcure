...
interface EmailHistoryItem {
  ...
  subject: string;
  email_body: string;
  sent_to?: string;  // Make this optional
...
export function EmailHistoryTab() {
  ...
  // Extract email from email body as fallback
  const extractEmailFromBody = (emailBody: string): string => {
    try {
      const toMatch = emailBody.match(/To:\s*([^<]+?)\s*<([^>]+)>/);
      ...
      console.error('Error extracting email from body:', error);
    }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {emails.map((email) => {
            const sentToEmail = email.sent_to || extractEmailFromBody(email.email_body);
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                      ...
                      <p>
                        <span className="font-medium">Sent to:</span> {sentToEmail}
                        {!email.sent_to && (
                          <span className="text-yellow-600 text-xs ml-2">(extracted from email)</span>
                        )}
                      </p>
